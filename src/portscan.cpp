...
void DisplayOptions() 
{
  ...
  optionDescriptions.push_back("Сканирование всех портов");
  optionDescriptions.push_back("Сканирование специальных портов");
  optionDescriptions.push_back("Сканирование общих портов");
...
void ScanAllPorts(std::string hostNameArg)
{
  ...
  if (buffer.size()==0)
  {
    std::cout << "Нет открытых портов" << std::endl;
  }
  else 
  {
    for (int i = 0; i < buffer.size(); i++)
      std::cout << "Порт " << buffer.at(i) << " открыт" << std::endl;
  }
}
...
int main(int argc, char* argv[])
{
    ...
    while (option<0||option>2) 
    {
      ...
      {
        std::cout << "Выбрана несуществующая операция.Повторите попытку" << std::endl;
        count++;
      }
    }
